...
abstract class ArticleDao : BaseDao<Article> {
    ...
    @Query("select guid, title, publish_date, is_read, is_favourite from articles")
    abstract fun loadArticlePreviews(): List<ArticlePreview>

    @Query("select guid, title, publish_date, is_read, is_favourite from articles a join articles_to_categories ac on a.guid = ac.article_id join categories c on ac.category_id = c.id where c.id = :categoryId")
